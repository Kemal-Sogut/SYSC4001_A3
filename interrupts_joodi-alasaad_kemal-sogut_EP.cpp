...
std::tuple<std::string, std::string > run_simulation(std::vector<PCB> list_processes) {
    ...
    while(!all_process_terminated(job_list) || job_list.empty()) {
        ...
        // Run if no process is currently running and there are processes in the ready queue
        ...
        if (running.state == RUNNING &&
            running.io_freq > 0 && 
            running.processed_time == running.io_freq &&
            running.remaining_time > 0)
        {
            ...
            idle_CPU(running);
        } else if (running.state == RUNNING) { // Process is running normally
...
int main(int argc, char** argv) {
    ...
    //Write the execution output to a file
